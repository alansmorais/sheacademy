...
// serviceId -> Stripe Price ID (TEST MODE)
const STRIPE_PRICE_MAPPING: Record<string, string> = {
...
async function startServer() {
  ...
  app.post('/api/stripe/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (bookingId) {
        ...
        // We need the googleScriptUrl. We'll try to read it from initialData or env
        // For simplicity in this proxy, we'll use an env var if available, or just log success
  ...
  app.get('/api/stripe/diagnostics', async (req, res) => {
    const testKey = req.query.testKey as string;
    const activeStripe = testKey ? new Stripe(testKey, { apiVersion: '2026-08-26.dahlia' }) : stripe;
    ...
    try {
      ...
      // @ts-ignore - Stripe SDK version types can be strict about arguments
      const account = await activeStripe.accounts.retrieve();
      
      // 2. Try to fetch the specific price
      let priceInfo = null;
      let priceExists = 'NO';
  ...
  app.post('/api/create-checkout-session', async (req, res) => {
    ...
    try {
      ...
      // Detailed error for price mismatches
